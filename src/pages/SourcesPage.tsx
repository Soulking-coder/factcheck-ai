  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-200">
      <div className="max-w-4xl mx-auto px-4 py-8 sm:py-12">
        ...
        >
          <div className="inline-flex p-3 glass border border-cyan-500/30 shadow-[0_0_20px_rgba(34,211,238,0.15)] rounded-2xl mb-4">
            <BookOpen className="w-7 h-7 text-cyan-400" />
          </div>
          <h1 className="text-3xl font-bold text-white glow-text mb-3">Our Methodology</h1>
        ...
        >
          <h2 className="text-xl font-bold text-white mb-4">How the AI Analysis Works</h2>
          <div className="space-y-4 text-sm text-slate-400 leading-relaxed">
            <p>
              FactCheck AI uses <strong className="text-cyan-300">Gemini 2.5 Flash</strong>, Google's highly capable language model, 
            ...
            <p>
              The system prompt is designed to be <strong className="text-cyan-300">completely non-partisan</strong> — 
            ...
            <p>
              Every response is <strong className="text-cyan-300">validated against a strict JSON schema</strong> to ensure 
        ...
        <motion.div
          ...
          className="mb-8"
        >
          <h2 className="text-xl font-bold text-white mb-4">What Gets Analyzed</h2>
          <div className="grid sm:grid-cols-2 gap-4">
            {analysisFactors.map((factor, i) => (
              ...
              >
                ...
                <h3 className="font-semibold text-slate-200 mb-2 text-sm">{factor.title}</h3>
        ...
        <motion.div
          ...
          className="glass-card rounded-2xl p-6 mb-8"
        >
          <h2 className="text-xl font-bold text-white mb-4">Understanding Verdicts</h2>
          <div className="space-y-4">
            {[
              { verdict: '✅ Likely True', color: 'text-emerald-400', description: 'The main claims appear factually accurate based on available evidence and align with expert consensus.' },
              { verdict: '⚠️ Mixed', color: 'text-amber-400', description: 'The content contains a mix of accurate and inaccurate or misleading information. Read critically and verify specific claims.' },
              { verdict: '❌ Likely False', color: 'text-red-400', description: 'The main claims appear to be false, misleading, or not supported by credible evidence. Do not share without thorough verification.' },
              { verdict: '❓ Unverifiable', color: 'text-slate-400', description: 'The claims cannot be independently verified. This may be because they involve predictions, unconfirmed events, or information that cannot be cross-referenced.' },
            ].map(item => (
              <div key={item.verdict} className="p-4 bg-slate-900/50 border border-slate-800 rounded-xl">
        ...
        <motion.div
          ...
          className="mb-8"
        >
          <h2 className="text-xl font-bold text-white mb-2">Trusted Fact-Checking Organizations</h2>
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            {trustedSources.map((source, i) => (
              <motion.a
                ...
                className="flex items-start gap-4 glass-card p-5 rounded-2xl hover:border-slate-700 hover:shadow-[0_0_20px_rgba(6,182,212,0.1)] transition-all group"
              >
                <div
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <span className="text-xs text-cyan-400 font-medium">{source.specialty}</span>
                </div>
              </motion.a>
            ))}
          </div>
        ...
        <motion.div
          ...
          className="bg-amber-950/30 border border-amber-500/30 rounded-2xl p-6"
        >
          <div className="flex items-center gap-2 mb-4">
            <AlertTriangle className="w-5 h-5 text-amber-600" />
            <h2 className="text-lg font-bold text-amber-300">Important Limitations</h2>
          </div>
          <div className="space-y-2.5">
            {limitations.map((limitation, i) => (
              <div key={i} className="flex items-start gap-2.5">
                <Info className="w-4 h-4 text-amber-500 shrink-0 mt-0.5" />
                <p className="text-sm text-amber-400">{limitation}</p>
              </div>
            ))}
          </div>
          <div className="mt-5 p-4 bg-amber-900/20 rounded-xl border border-amber-500/20">
            <p className="text-sm font-semibold text-amber-300 mb-1">Our Core Principle</p>
